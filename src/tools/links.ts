...
export function createLinksTools(store: StoreProvider) {
  ...
  const addLinkTool = createTool({
    ...
    inputSchema: z.object({
      fromSlug: z.string().describe('The slug of the page where the link originates.'),
      toSlug: z.string().describe('The slug of the target page.'),
      linkType: z.string().optional().describe('The type of relationship (e.g. reference, mention).'),
      context: z.string().optional().describe('The context in which the link was made.')
    }),
    execute: async (inputData) => {
      await store.addLink(inputData.fromSlug, inputData.toSlug, inputData.linkType || '', inputData.context || '');
      return { success: true, message: `Linked '${inputData.fromSlug}' to '${inputData.toSlug}'` };
    }
  });
  ...
  const removeLinkTool = createTool({
    ...
    execute: async (inputData) => {
      ...
      return { success: true, message: `Removed link from '${inputData.fromSlug}' to '${inputData.toSlug}'` };
    }
  });
