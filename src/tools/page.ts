...
export function createPageTools(store: StoreProvider) {
  const readPageTool = createTool({
    id: 'read-page',
    description: 'Read the full compiled content and frontmatter of a specific page.',
  ...
  const pageInfoTool = createTool({
    ...
    inputSchema: z.object({
      slug: z.string().describe('The unique slug of the page to retrieve info for.')
    }),
    execute: async (inputData) => {
  ...
  const addTagTool = createTool({
    ...
    execute: async (inputData) => {
      ...
      return { success: true, message: `Tag '${inputData.tag}' added to '${inputData.slug}'` };
    }
  });
  ...
  const removeTagTool = createTool({
    ...
    execute: async (inputData) => {
      ...
      return { success: true, message: `Tag '${inputData.tag}' removed from '${inputData.slug}'` };
    }
  });
