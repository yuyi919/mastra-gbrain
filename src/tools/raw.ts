...
export function createRawDataTools(store: StoreProvider) {
  const getRawDataTool = createTool({
    id: 'get-raw-data',
    description: 'Get raw external data (like fetched HTML or JSON API responses) stored for a specific page.',
    inputSchema: z.object({
      slug: z.string().describe('The unique slug of the page.'),
      source: z.string().describe('The source identifier (e.g. github, twitter, wikipedia).')
    }),
    execute: async (inputData) => {
      const rawData = await store.getRawData(inputData.slug, inputData.source);
      if (!rawData) {
        return { error: `Raw data for source '${inputData.source}' not found on page '${inputData.slug}'` };
      }
      return {
        slug: inputData.slug,
  ...
  const putRawDataTool = createTool({
    id: 'put-raw-data',
    description: 'Store raw external data (like fetched HTML or JSON API responses) for a specific page.',
    inputSchema: z.object({
      slug: z.string().describe('The unique slug of the page.'),
      source: z.string().describe('The source identifier (e.g. github, twitter, wikipedia).'),
    ...
    execute: async (inputData) => {
      ...
      return { success: true, message: `Stored raw data from '${inputData.source}' for page '${inputData.slug}'.` };
    }
  });
