...
import { LlamaEmbeddingProvider } from './llama-embedder.ts';
...
export function createDefaultEmbedder(options: DefaultEmbedderOptions = {}): EmbeddingProvider {
  ...
  const kind: DefaultEmbedderKind =
    options.kind ??
    envKind ??
    (options.llama?.modelPathEn || options.llama?.modelPathZh || envModelPathEn || envModelPathZh ? 'llama' : 'dummy');
  ...
  const dimension = options.dummy?.dimension ?? readDummyDimensionFromEnv() ?? 1536;
