...
export async function checkBacklinks(
  ...
  missingCount: number;
  missingBacklinks: Map<string, Set<string>>;
}> {
    ...
    if (actuallyMissing.size > 0) {
      ...
      if (fix) {
        ...
        for (const s of actuallyMissing) {
          ...
          newEntries.push(
            `- **${today}** | Referenced in [${title}](${relPath})`
          );
        }
        ...
        if (content.includes("## Timeline")) {
          ...
          if (nextSection) {
            const insertIdx =
              parts[0].length + "## Timeline".length + nextSection.index!;
            content =
        ...
        if (!dryRun) {
          ...
          console.log(
            `🔍 [Dry Run] Would fix ${target} by appending to Timeline:\n${entriesStr}`
          );
        }
      }
    } else {
...
if (import.meta.main) {
  ...
  const dir =
    args.filter((a) => a !== "check" && a !== "fix" && a !== "--dry-run")[0] ||
    ".";

  if (mode !== "check" && mode !== "fix") {
    console.error(
      "Usage: bun run src/scripts/backlinks.ts <check|fix> <directory> [--dry-run]"
    );
    process.exit(1);
  }
  ...
  checkBacklinks(resolve(dir), mode === "fix", dryRun)
    .then(({ missingCount }) =>
      process.exit(missingCount > 0 && mode === "check" ? 1 : 0)
    )
    .catch(console.error);
}
