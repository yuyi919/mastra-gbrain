  ...
  try {
    ...
    for (const [table, details] of Object.entries(report.tableDetails)) {
      if (details.ok) {
        checks.push({
          name: `table_${table}`,
          status: "ok",
          message: `Table '${table}': OK (${details.rows} rows)`,
        });
      } else {
        checks.push({
          name: `table_${table}`,
          status: "fail",
          message: `Table '${table}': Missing or Corrupted (${details.error})`,
        });
      }
    }
    ...
    if (report.ftsOk) {
      checks.push({
        name: "fts5",
        status: "ok",
        message: "FTS5 Index: Integrity OK",
      });
    } else {
      checks.push({
        name: "fts5",
        status: "fail",
        message: "FTS5 Index: Failed integrity check",
      });
    }
    ...
    if (embedded >= 0) {
      ...
      if (total > 0 && embedded < total) {
        checks.push({
          ...
          status: "warn",
          message: `Vector Coverage: ${coverage}% (${embedded}/${total} chunks embedded)`,
        });
      } else {
        checks.push({
          name: "vector_coverage",
      ...
      checks.push({
        ...
        status: "warn",
        message:
          "Vector Coverage: Could not determine vector count. Check VectorStore configuration.",
      });
    }
    ...
    if (report.schemaVersion) {
      ...
      if (ok) {
        checks.push({
          name: "schema_version",
          status: "ok",
          message: `Schema Version: ${current} (latest: ${latest})`,
        });
      } else {
        checks.push({
          name: "schema_version",
          status: "warn",
          message: `Schema Version: ${current}, latest is ${latest}. Migration might be needed.`,
        });
      }
    }
  } catch (error: any) {
