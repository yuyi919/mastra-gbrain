...
import type { StoreProvider, EmbeddingProvider } from '../../src/store/interface.ts';
import { createHash } from 'crypto';
import { parseMarkdown } from '../../src/markdown.ts';

function mockStore(overrides: Partial<Record<string, any>> = {}): StoreProvider & { _calls: any[] } {
...
describe('ingestion workflow', () => {
  test('imports valid markdown content (noEmbed)', async () => {
    ...
    const workflow = createIngestionWorkflow({ store, embedder: mockEmbedder() });
...
`;
    ...
    const workflow = createIngestionWorkflow({ store, embedder: mockEmbedder() });
    const run = await workflow.createRun();
    const res = await run.start({ inputData: { relativePath: 'concepts/same.md', content, noEmbed: true } });
    expect(res.status).toBe('success');
    if (res.status === 'success' && 'result' in res) {
      expect((res.result as any).status).toBe('skipped');
    }
    expect((store as any)._calls.some((c: any) => c.method === 'putPage')).toBe(false);
  });
...
`,
  ...
  test('rejects oversized content as skipped', async () => {
    const store = mockStore();
    const workflow = createIngestionWorkflow({ store, embedder: mockEmbedder(), maxBytes: 10 });
    const run = await workflow.createRun();
    const res = await run.start({
      inputData: {
        slug: 'big',
...
`,
    ...
    const upsert = (store as any)._calls.find((c: any) => c.method === 'upsertChunks');
