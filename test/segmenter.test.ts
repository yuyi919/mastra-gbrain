...
import { countWords, extractWordsForSearch, getSegments } from '../src/segmenter.ts';
...
describe('Segmenter Utilities', () => {
  ...
  test('extractWordsForSearch produces space-separated words', () => {
    ...
    expect(extractWordsForSearch('这是一个中文测试。')).toBe('这 是 一个 中文 测试');
    expect(extractWordsForSearch('これは日本語のテストです。')).toBe('これ は 日本語 の テスト です');
    
    // It should ignore punctuation
    expect(extractWordsForSearch('Hello, world. Is this a test?')).toBe('Hello world Is this a test');
