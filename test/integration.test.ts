...
import { bulkImport } from '../src/scripts/import.ts';
import { LibSQLStore } from '../src/store/libsql.ts';
import { resolve } from 'node:path';
import { createSearchTool } from '../src/tools/search.ts';
import { createPageTools } from '../src/tools/page.ts';
import { createTimelineTool } from '../src/tools/timeline.ts';
import { createLinksTools } from '../src/tools/links.ts';
import { createListPagesTool } from '../src/tools/list.ts';
import { DummyEmbeddingProvider } from '../src/store/dummy-embedder.ts';

let testStore: LibSQLStore;

beforeAll(async () => {
  // Use a specific test database for integration
  testStore = new LibSQLStore({ url: 'file:./tmp/test-integration.db', dimension: 1536 });
...
afterAll(async () => {
  ...
  import('node:fs').then(fs => {
    try { fs.unlinkSync('./tmp/test-integration.db'); } catch (e) {}
  });
});
...
test('Bulk import and Extended Tools Integration', async () => {
  ...
  const { imported, skipped, failed } = await bulkImport(docsDir, testStore, embedder);
  ...
  const searchResult: any = await searchTool.execute!({ query: 'Mastra', limit: 5 }, {} as any);
  expect(searchResult.results.length).toBeGreaterThan(0);
  expect(searchResult.results[0].slug).toBe('concepts/mastra');

  // 3. Test page tools (pageInfo, readPage, listPages)
  const pageInfo: any = await pageInfoTool.execute!({ slug: 'concepts/gbrain' }, {} as any);
  ...
  const pageContent: any = await readPageTool.execute!({ slug: 'concepts/gbrain' }, {} as any);
  expect(pageContent.compiled_truth).toContain('GBrain 是一个先进的个人知识库系统');
  expect(pageContent.title).toBe('GBrain Architecture');

  const allConceptPages: any = await listPagesTool.execute!({ type: 'concept' }, {} as any);
  expect(allConceptPages.pages.length).toBe(2);
  expect(allConceptPages.pages.some((p: any) => p.slug === 'concepts/mastra')).toBe(true);

  // 4. Test timelineTool
  const timeline: any = await timelineTool.execute!({ slug: 'people/garry' }, {} as any);
  ...
  const links: any = await linksTool.execute!({ slug: 'people/garry' }, {} as any);
