...
beforeAll(async () => {
  ...
  await store.putPage('page-a', { type: 'concept', title: 'Page A', frontmatter: {}, compiled_truth: '', timeline: '', content_hash: 'a' });
  await store.putPage('page-b', { type: 'concept', title: 'Page B', frontmatter: {}, compiled_truth: '', timeline: '', content_hash: 'b' });
});

afterAll(async () => {
  await store.dispose();
  import('fs').then(fs => {
    try { fs.unlinkSync('./tmp/test-ext.db'); } catch (e) {}
  });
});
...
test('Timeline entries management', async () => {
  await store.upsertTimelineEntries('page-a', [
    { date: '2025-01-01', source: 'test', summary: 'New year', detail: 'Started 2025' }
  ]);
