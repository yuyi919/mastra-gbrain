...
import { slugifyPath, slugifySegment } from '../src/slug.ts';
...
describe('slug utility', () => {
  ...
  test('slugifyPath processes file paths correctly', () => {
    expect(slugifyPath('src/components/button.md')).toBe('src/components/button');
    expect(slugifyPath('src/components/button.mdx')).toBe('src/components/button');
