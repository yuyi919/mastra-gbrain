...
import { embedStale } from '../../src/scripts/embed.ts';
...
beforeAll(async () => {
  ...
  const db = store['db'];
  const pageResult = db.query('SELECT id FROM pages WHERE slug = ?').get('stale-slug') as { id: number };
  const pageId = pageResult.id;

  db.query(`INSERT INTO content_chunks (page_id, chunk_index, chunk_text, chunk_source, token_count) 
            VALUES (?, 0, 'First stale chunk', 'compiled_truth', 3)`).run(pageId);
  db.query(`INSERT INTO content_chunks (page_id, chunk_index, chunk_text, chunk_source, token_count) 
            VALUES (?, 1, 'Second stale chunk', 'compiled_truth', 3)`).run(pageId);
...
test('embedStale processes un-embedded chunks', async () => {
  ...
  const db = store['db'];
  const chunks = db.query('SELECT embedded_at FROM content_chunks WHERE chunk_text LIKE ?').all('%stale chunk%') as { embedded_at: string | null }[];
