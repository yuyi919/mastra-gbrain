...
import { createIngestTool } from '../src/tools/ingest.ts';
...
beforeAll(async () => {
  testStore = new LibSQLStore({ url: 'file:./tmp/test-tools.db', dimension: 1536 });
...
afterAll(async () => {
  ...
  import('fs').then(fs => {
    try { fs.unlinkSync('./tmp/test-tools.db'); } catch (e) {}
  });
});
