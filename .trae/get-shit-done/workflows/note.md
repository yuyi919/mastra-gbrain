...
- **Global scope**: `/workspace/.trae/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspace/.trae/notes/`)
...
2. Glob `/workspace/.trae/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `/workspace/.trae/notes/` — works in any directory
