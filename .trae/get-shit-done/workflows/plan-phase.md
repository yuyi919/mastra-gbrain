...
@/workspace/.trae/get-shit-done/references/revision-loop.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/workspace/.trae/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
**If found:**
1. Read template: `/workspace/.trae/get-shit-done/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/workspace/.trae/get-shit-done/references/tdd.md to all eligible tasks:
